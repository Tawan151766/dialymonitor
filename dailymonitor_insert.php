...
if (isset($_POST["operation"])) {
  ...
  if ($_POST["operation"] == "Edit") {
    ...
    if ($file && ($fileType == "pdf" || $fileType == "jpg" || $fileType == "jpeg")) {
      ...
      $fileName   = date('dmYHis');
      $fileName   = $_POST["subject_id"] . "@" . $fileName . "." . $fileType;
      $uploadFile = $path . "" . $fileName;
      copy($file, $uploadFile);
      $attached = $fileName;
      ...
      $attached = $_POST['pdf_attached'];

    }

    if ($_POST['delete_doc'] != "" && $_POST['delete_doc'] == $_POST['pdf_attached']) {

      $attached    = "";
      $delete_file = $path . $_POST["delete_doc"];
      unlink($delete_file);

    }

    $statement = $connection->prepare("UPDATE dailymonitor SET
